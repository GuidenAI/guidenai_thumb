...
const docsSimpleOceanLayoutConfig = z.object({
  Page: z.string(),
...
export type DocsSimpleOceanLayoutConfig = z.infer<typeof docsSimpleOceanLayoutConfig>;

const Component: React.FC<{ config: DocsSimpleOceanLayoutConfig & { isServerSide?: boolean } }> = ({ config }) => {
  const url =
    (config.Url ?? "").trim() === ""
      ? "guidenai.com"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(245.93deg, #14C4B7 0%, #3BB0E1 36.63%, #5E7BEA 100%)",
        }}
      />
      <div
        tw="absolute inset-0"
...
export const docsSimpleOceanLayout: ILayout<typeof docsSimpleOceanLayoutConfig> = {
  name: "docs-simple-ocean",
  config: docsSimpleOceanLayoutConfig,
  properties: [
    {
      type: "text",
      name: "Page",
